            ...
            <div class="q-menu-section">
                <q-list>
                    <template v-for="(menu, index) in menuItems" :key="index">
                        <q-item clickable v-ripple v-if="menu.show && (!('roleAccess' in menu) || menu.roleAccess === currentUser.role.name)" :class="currentSection.name == menu.name ? 'q-active' : ''">
                            <Link :href="menu.url">
...
export default {
    ...
    setup(props, {emit}) {
        ...
        const currentUser = page.props.currentUser.data;
        const goToRoute = (url) => {
            router.visit(url, {
                method: 'get',
                preserveState: true,
