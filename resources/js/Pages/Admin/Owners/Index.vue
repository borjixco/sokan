                    ...
                    <div class="rb-actions-section">
                        <div class="rb-add-section">
                            <excel-button
                                class="rb-action"
                                label="خروجی مالکین"
                                :route="route('admin.owners.excel')"
                                slug="خروجی-مالکین"
                                title="فیلتر خروجی مالکین"
                                btn-title="خروجی"
                                card-class="full-width"
                                :use-dialog="false"
                            />
                            <q-btn class="rb-action" icon="print" label="پرینت" @click="printHandle"/>
                            <Link :href="route('admin.owners.create')">
                                <q-btn type="submit" class="rb-action" icon="add" label="افزودن مالک"/>
                            </Link>
                        </div>
                    </div>
                </div>
            ...
            <q-card class="q-mb-lg">
                ...
                >
                    ...
                    <template v-slot:body="props">
                        <q-tr align="center">
                            <q-td>
                                <span class="q-unit" v-for="(unitNumber,index) in props.row.ownerUnits" :key="index">
                                    {{ unitNumber }}
                                </span>
                            </q-td>
                            ...
                            <q-td class="rb-eye">
                                <Link :href="route('admin.owners.edit',props.row.id)">
...
export default {
    ...
    setup() {
        ...
        const searchQuery = debounce(() => {
            ...
            router.get(route('admin.owners'), {search: search.value}, {
                preserveState: true,
                preserveScroll: true,
                onSuccess: (response) => {
        ...
        const perPageHandle = (value) => {
            perPage.value = value;
            isLoading.value = true
            router.get(route('admin.owners'), {search: search.value, perPage: perPage.value}, {
                preserveState: true,
                preserveScroll: true,
                onSuccess: (response) => {
                    isLoading.value = false
        ...
        const uploadExcel = ({xhr}) => {
            ...
            return goToRoute(route('admin.owners'))
        }
